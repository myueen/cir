...
for itr = 1 : opts.mxitr
    ...
    %    display(num2str(itr))
    %    cck = X*X.';
    %    display(num2str(cck(1,1:3)))
    %end
    XP = X;     FP = F;   dtXP = dtX;   nrmGP = nrmG;
    %display(num2str(FP(1,1)))
    %display(num2str(dtXP(1,1)))
    %display(num2str(nrmGP(1,1)))
    ...
    %display(['Qp = ', num2str(Qp)])
    %display(['Q = ', num2str(Q)])
    %display(['Cval = ', num2str(Cval)])
end
tiempo = toc(tstart);
...
out.itr = itr;
out.time = tiempo;
%display(['Size of crit = ',num2str(size(crit),1)])
%display(['Number of iteration = ', num2str(itr)])
end
